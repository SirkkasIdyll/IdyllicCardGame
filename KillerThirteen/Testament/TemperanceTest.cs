...
public class TemperanceTest
{
    ...
    public async Task ComponentManagerTest()
    {
        ...
        // Adds component
        AssertBool(_componentManager.TryAddComponent<CardComponent>(testNode)).AppendFailureMessage(
            "Failed to add component to node.").IsTrue();
        
        // Gets component
        AssertBool(_componentManager.TryGetComponent<CardComponent>(testNode, out var cardComponent)).AppendFailureMessage(
            "Failed to retrieve component from node after adding it.").IsTrue();
        AssertObject(cardComponent).IsNotNull();
        
        // Gets component again
        // _componentManager.GetNodesWithComponent<CardComponent>(out var list);
        ...
        _componentManager.RemoveComponent<CardComponent>(testNode);
        await Task.Delay(100);
        
        // Has component
        AssertBool(_componentManager.HasComponent<CardComponent>(testNode)).AppendFailureMessage(
